...
class WebViewActivity : AppCompatActivity() {
    ...
    class WebAppInterface(private val activity: WebViewActivity) {
        ...
        @JavascriptInterface fun submitExam(scoreNotUsed: Double, responseJson: String) {
            ...
            activity.runOnUiThread {
                try {
                    activity.isExamActive = false // MATIKAN FLAG SEGERA
                    activity.isExamSubmitted = true // TANDAI SUDAH SUBMIT
                    val examData = JSONObject(examDataStr)
                    var questions = examData.optJSONArray("questions") ?: examData.optJSONObject("data")?.optJSONArray("questions")
                    ...
                    val finalScore = if (totalQuestions > 0) (correctCount.toDouble() / totalQuestions.toDouble()) * 100.0 else 0.0
                    val result = JSONObject().apply {
                        put("subject_id", examId)
                        put("nisn", CacheManager.getStudentInfo()?.optString("nisn"))
                        put("score", finalScore)
                        // --- PERBAIKAN: Tambahkan dua baris ini agar data terbaca oleh Dashboard dan Server ---
    ...
    override fun onWindowFocusChanged(hasFocus: Boolean) {
        ...
        if (!hasFocus && isExamActive && !isShowingAppDialog && !isExamSubmitted && isScreenOn) {
