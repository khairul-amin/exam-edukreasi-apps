...
class CsvDataFetcher(private val context: Context) {
    ...
    // MEMASTIKAN HANYA ADA 3 MODE: "online", "on-lan", "semi-offline"
    fun setMode(isOffline: Boolean) = prefs.edit().putString(PREF_APP_MODE, if (isOffline) "on-lan" else "online").apply()

    fun setAppMode(mode: String) {
        // INTERCEPTOR: Jika QR code lama mengirim "offline", paksa ubah jadi "on-lan"
        val finalMode = if (mode == "offline") "on-lan" else mode
        prefs.edit().putString(PREF_APP_MODE, finalMode).apply()
    }

    fun getAppMode(): String = prefs.getString(PREF_APP_MODE, "online") ?: "online"

    // Validasi Offline yang strict hanya untuk on-lan dan semi-offline
    fun isOfflineMode(): Boolean = prefs.getString(PREF_APP_MODE, "online").let {
        it == "semi-offline" || it == "on-lan"
    }
    ...
    suspend fun loginStudent(studentId: String): JSONObject {
        return withContext(Dispatchers.IO) {
            try {
                // HANYA MENGGUNAKAN API WORKER (Semi-Offline)
                // Kirim "id" dan "nisn" sekaligus agar sesuai dengan apapun yang diminta backend
                ...
                val request = Request.Builder()
                    .url("https://exam-edukreasi-api.edukreasi.workers.dev/api/student/login")
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    if (response.isSuccessful && json.optString("status") == "success") {
                        ...
                        return@withContext JSONObject().apply {
                            put("status", "error")
                            put("message", json.optString("message", "Login ditolak server (Cek ID Siswa)"))
                        }
                    }
                }
            } catch (e: Exception) {
                // SAAT TIDAK ADA INTERNET: Kembalikan error jaringan.
                // Tidak perlu fallback ke CSV karena ujian murni memakai CacheManager.
                return@withContext JSONObject().apply {
                    put("status", "error")
                    put("message", "Tidak ada koneksi internet. Menggunakan mode offline.")
                }
            }
        }
    }
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (token.isNullOrEmpty()) {
                    return@withContext JSONObject().apply {
                        put("status", "error")
                        put("message", "Token tidak tersedia (Silakan login ulang)")
                    }
                }

                val request = Request.Builder()
                    .url("https://exam-edukreasi-api.edukreasi.workers.dev/api/student/subjects")
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    Log.d("DebugOffline", "API HTTP ${response.code} | Body: $bodyStr")

                    // --- TAMBAHKAN DETEKSI 401 DI SINI ---
                    if (response.code == 401) {
                        return@withContext JSONObject().apply {
                            put("status", "error")
                            put("is_token_expired", true) // Flag khusus untuk memicu auto-login
                            put("message", "Sesi siswa tidak valid atau kedaluwarsa.")
                        }
                    }
                    // ------------------------------------

                    // Coba parse body ke JSON dengan aman
                    val json = try {
                        JSONObject(bodyStr)
                    } catch(e: Exception) {
                        JSONObject()
                    ...
                    // Jika sukses dan status server juga success
                    if (response.isSuccessful && json.optString("status").equals("success", ignoreCase = true)) {
                        return@withContext json
                    } else {
                        // JANGAN DITELAN ERROR-NYA: Ambil pesan error dari server
                        val serverMessage = json.optString("message", "Ditolak server atau format salah")
                        return@withContext JSONObject().apply {
                            put("status", "error")
                            put("message", "Error ${response.code}: $serverMessage")
                        }
                    }
                }
            } catch (e: Exception) {
                Log.e("DebugOffline", "Exception di fetchSubjects: ${e.message}")
                JSONObject().apply {
                    put("status", "error")
                    put("message", "Gagal koneksi internet: ${e.message}")
                }
            }
        }
    }
